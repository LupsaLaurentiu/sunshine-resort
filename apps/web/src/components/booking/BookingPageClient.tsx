...
const DEFAULT_ADULTS_PER_ROOM = 2;
const FALLBACK_ROOM_IMAGE = "/hero-mockup.jpg";
...
function normalizeRoomSlug(slug: string): string {
  return slug
    .trim()
    .toLowerCase()
    .replace(/^apartament-/, "")
...
export function BookingPageClient() {
  ...
  const availableRooms = useMemo<BookingRoom[]>(() => {
    if (!availability) {
      return [];
    }

    return availability.roomTypes
      .filter((roomType) => roomType.availableUnits > 0)
      .map((roomType) => {
        const normalizedBackendSlug = normalizeRoomSlug(roomType.slug);

        const publicContent = publicRooms.find(
          (room) =>
            normalizeRoomSlug(room.slug) === normalizedBackendSlug,
        );
        ...
        return {
          ...
          description:
            backendDescription?.trim() || fallbackDescription,

          image: publicContent?.image ?? FALLBACK_ROOM_IMAGE,
          ...
          averagePricePerNight: roomType.averagePricePerNight,

          hasPromotion: roomType.hasPromotion,
  ...
  async function handleSubmitReservation() {
    ...
    const rooms = availableRooms
      .map((room) => ({
        ...
        quantity: selectedRooms[room.slug] ?? 0,
        adultsPerRoom: Math.min(
    ...
    const response = await submitReservation({
      ...
      guestNotes: guest.message.trim() || undefined,
    });
  ...
  return (
    <main className="bg-[#050505] text-[#f5f2eb]">
      <BookingSuccess reservation={reservation} />
    </main>
  );
}
